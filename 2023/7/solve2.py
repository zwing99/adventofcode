...
card_rank = {"A": 14, "K": 13, "Q": 12, "J": 0, "T": 10, "9": 9, "8": 8, "7": 7, "6": 6, "5": 5, "4": 4, "3": 3, "2": 2}
...
for line in lines:
    ...
    if len(h) == 1:
        ...
        if 4 in h.values():
            if 0 in h:
                rank = 7 # must be 5 of kind with J
            else:
        ...
        if 3 in h.values():
            if 0 in h:
                rank = 6 # must be 4 of kind with J
            else:
            ...
            if 0 in h:
                if h[0] == 2:
                    ...
                    rank = 5 # must be full house with J
            else:
                rank = 3 # must be 2 pairs
    elif len(h) == 4:
        if 0 in h:
            rank = 4 # must be 3 of kind with J
        else:
            rank = 2 # must be 1 pair
    else:
        if 0 in h:
            rank = 2 # must be 1 pair with J
        else:
            rank = 1 # must be high card
    hand_rank = [rank] + [card_rank[c] for c in hand]
    #print("Hand: {}, Bet: {}, Rank: {}".format(hand, bet, hand_rank))
    hands.append(Hand(hand, hand_rank, int(bet)))
