...
def all_full_path_options(full_path_options):
    ...
    for full_path_r1 in full_path_options:
        ...
        all_full_path_next_options.extend(full_path_next_options)
...
for line in lines:
    ...
    def internal_dfs(path, i, memo):
        ...
        if i >= 2:
            ...
            print(path)
            return
        for new_path in all_full_path_options([path]):
            # print(new_path)
            internal_dfs(new_path, i + 1, memo)

    memo = {}
    for step in steps:
        internal_dfs(step, 0, memo)

    best -= 1
    print(best, int(full_line.strip("A")))
    total += best * int(full_line.strip("A"))
