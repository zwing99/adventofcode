...
def process_packet(packet: str, depth=0):
    ...
    if packet_typ == 4:
        ...
        return packet, packet_ver, packet_typ, version_sum
    else:
        ...
        if l_bit == "0":
            ...
            while True:
                sub_packets, sub_packet_ver, sub_packet_type, v_sum = process_packet(
                    sub_packets[:], depth + 1
                )
                version_sum += v_sum
            ...
            for _ in range(num_packets):
                packet, sub_packet_ver, sub_packet_type, v_sum = process_packet(
                    packet[:], depth + 1
                )
                version_sum += v_sum
        return packet, packet_ver, packet_typ, version_sum
...
for line in lines:
    ...
    print(line)
    print(b)
    left_over, _, _, version_sum = process_packet(b)
    print(left_over)
    print(version_sum)
    print()
