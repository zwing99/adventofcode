...
grid = [[] for x in range(6)]
# face 1
for line in grid_lines[:SIZE_CUBE]:
    row = tuple([to_map(x) for x in line.strip()])
    grid[0].append(row)
# face 2,3,4
for line in grid_lines[SIZE_CUBE : SIZE_CUBE * 2]:
    row = tuple([to_map(x) for x in line.strip()])
    grid[1].append(row[:SIZE_CUBE])
    grid[2].append(row[SIZE_CUBE : SIZE_CUBE * 2])
    grid[3].append(row[SIZE_CUBE * 2 :])
# face 5,6
for line in grid_lines[SIZE_CUBE * 2 :]:
    row = tuple([to_map(x) for x in line.strip()])
    grid[4].append(row[:SIZE_CUBE])
    grid[5].append(row[SIZE_CUBE:])
for f in grid:
    print(f)
exit()
grid = tuple(grid)
